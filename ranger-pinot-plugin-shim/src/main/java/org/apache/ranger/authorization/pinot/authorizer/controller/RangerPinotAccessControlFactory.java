...
/**
 * This is the class Pinot's controller actually instantiates by reflection (configured under
 * {@code controller.admin.access.control.factory.class}), so it must be loaded by Pinot's own
 * classloader and directly implement Pinot's {@link AccessControlFactory}.
 *
...
public class RangerPinotAccessControlFactory implements AccessControlFactory {
    ...
    public RangerPinotAccessControlFactory() {
        ...
        try {
            ...
            LOG.error("Error enabling RangerPinotAccessControlFactory (controller)", e);

            throw new IllegalStateException("Error enabling RangerPinotAccessControlFactory (controller)", e);
        }
